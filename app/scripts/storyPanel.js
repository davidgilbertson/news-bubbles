...
NB.StoryPanel = (function() {
    ...
    function getGifvHtml(gifVUrl) {
        var webm = gifVUrl.replace(/.gifv$/, '.webm');
        var mp4 = gifVUrl.replace(/.gifv$/, '.mp4');

        var html = [
            '<a href="' + gifVUrl + '" target="_blank">',
    ...
    function renderRdt(story) {
        ...
        } else if (story.url.match(/gifv$/)) { //any old image link, might be imgur
            story.content = getGifvHtml(story.url);
            done(true);
